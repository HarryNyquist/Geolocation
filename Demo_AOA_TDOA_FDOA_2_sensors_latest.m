function fig = Demo_AOA_TDOA_FDOA_2_sensors()
% fig=book2_ex2_2()
%
...
for i = 1:numSimulations
    ...
    % ML Solution
    x_ml_results(:, i) = hybrid.mlSoln(x_aoa, x_tdoa, x_fdoa, v_fdoa, zeta_noisy, cov_z, x_ctr, grid_size, epsilon);
    
    % GD Solution
    x_gd_results(:, i) = hybrid.gdSoln(x_aoa, x_tdoa, x_fdoa, v_fdoa, zeta_noisy, cov_z, x_init, [], [], epsilon, max_num_iterations, force_full_calc, plot_progress);
    
    % LS Solution
    x_ls_results(:, i) = hybrid.lsSoln(x_aoa, x_tdoa, x_fdoa, v_fdoa, zeta_noisy, cov_z, x_init, epsilon, max_num_iterations, force_full_calc, plot_progress);
end
...
plot(x_source(1), x_source(2), 'ko', 'MarkerSize', 10, 'DisplayName', 'True Source');
...
resolved_psi_est = [];
angle_error = 0.09; % Coarse estimate error in degrees
for i = 1:k
