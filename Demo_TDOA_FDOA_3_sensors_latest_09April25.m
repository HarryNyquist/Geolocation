function fig = Demo_TDOA_FDOA_3_sensors()
% fig=book2_ex2_2()
%
...
%x_aoa = [4; 0]*1e3;
x_tdoa = [0, 20, 40; 0, 0, 0]*1e3;
x_fdoa = [0, 20, 40; 0, 0, 0]*1e3;

% x_tdoa = [0, 80; 0, 0]*1e3;
% x_fdoa = [0, 80; 0, 0]*1e3;
v_fdoa = [1, 1, 1.2; 1, 1, 0.7]*71 ;%*sqrt(.5)*100; % 100 m/s, at -45 deg heading
...
err_time = (50e-9); % 20 ns timing error
...
grid_res = 100;  % meters, grid resolution
...
x_ls_full = zeros(2,max_num_iterations,numSimulations);
%x_grad_full = zeros(2,max_num_iterations,numSimulations);
...
for i = 1:numSimulations
    ...
    % GD Solution
    %[x_gd_results(:, i), x_gd_full(:,:,i)] = hybrid.gdSoln_TF(x_tdoa, x_fdoa, v_fdoa, zeta_noisy, cov_z, x_init, [], [], epsilon, max_num_iterations, force_full_calc, plot_progress);
...
%x_gd_dev = x_source - x_gd_full(:,:,10);
x_ls_dev = x_source - x_ls_full(:,:,10);
distance_error_ls = sqrt(x_ls_dev(1,:).^2 + x_ls_dev(2,:).^2);
%distance_error_gd = sqrt(x_gd_dev(1,:).^2 + x_gd_dev(2,:).^2);
figure;
%subplot(2,2,4);
loglog(1:max_num_iterations, distance_error_ls, 'm-','LineWidth',1.6, 'DisplayName','LS error');hold on;
%plot(1:max_num_iterations, distance_error_gd, '.g-','LineWidth',1.6,'DisplayName','GD error');
xlabel('Num of Iterations');
ylabel('distance Error');
title('Number of iterations vs location error');
...
plot(x_tdoa(1, :), x_tdoa(2, :), 'ks', 'DisplayName','TDOA/FDOA Sensor');
...
subplot(1,2,1);
%bar(abs_dev_x, 'b');
histogram(x_ls_dev_x_dir_source, 'BinWidth', 40,'FaceColor', 'b')
...
subplot(1,2,2);
%bar(abs_dev_y, 'r');
histogram(x_ls_dev_y_dir_source, 'BinWidth', 80,'FaceColor', 'g')
title('Deviations in Y from source');
xlabel('deviation in meters');
ylabel('Num of deviations');
legend('Deviations', 'Location', 'best');
grid on;
...
%subplot(2,2,3);
figure;
plot(x_ls_dev_x_dir_source, x_ls_dev_y_dir_source, '*r', 'MarkerSize',2, 'DisplayName', 'LS-estimate errors');
...
%subplot(2,2,4);
figure;
scatter(x_ls_dev_x_dir_source, x_ls_dev_y_dir_source, 'b', 'filled','DisplayName', 'LS-estimate errors');
hold on;

% Plot CEP50 (50% circle)
x_CEP50_source = CEP50_source * cos(theta_dev);
y_CEP50_source = CEP50_source * sin(theta_dev);
plot(x_CEP50_source, y_CEP50_source, 'r-', 'LineWidth', 2,'DisplayName', 'CEP50-points');

% Plot CEP80 (80% circle)
x_CEP80_source = CEP80_source * cos(theta_dev);
y_CEP80_source = CEP80_source * sin(theta_dev);
plot(x_CEP80_source, y_CEP80_source, 'g--', 'LineWidth', 2, 'DisplayName', 'CEP80-points');
