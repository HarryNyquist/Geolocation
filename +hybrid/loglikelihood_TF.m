function ell = loglikelihood_TF(x_tdoa,x_fdoa,v_fdoa,zeta,C,x_source,tdoa_ref_idx,fdoa_ref_idx)
...
% test_idx_vec = cat(2,1:tdoa_test_idx_vec, n_tdoa + fdoa_test_idx_vec);
% %ref_idx_vec = cat(2,nan(1,n_aoa), n_aoa + tdoa_ref_idx_vec, n_aoa + n_tdoa + fdoa_ref_idx_vec);
% ref_idx_vec = cat(2,nan(1,n_tdoa), n_tdoa + fdoa_ref_idx_vec);
% % For now, we assume the AOA is independent of TDOA/FDOA
% C_tilde = utils.resampleCovMtx(C, test_idx_vec, ref_idx_vec);

test_idx_vec = [tdoa_test_idx_vec, n_tdoa + fdoa_test_idx_vec];
ref_idx_vec = [tdoa_ref_idx_vec, n_tdoa + fdoa_ref_idx_vec];
C_tilde = utils.resampleCovMtx(C, test_idx_vec, ref_idx_vec);
...
for idx_source = 1:n_source_pos
    ...
    z = hybrid.measurement_TF(x_tdoa,x_fdoa,v_fdoa, x_i, tdoa_ref_idx, fdoa_ref_idx);
