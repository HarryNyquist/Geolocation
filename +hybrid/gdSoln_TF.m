function [x,x_full] = gdSoln_TF(x_tdoa, x_fdoa, v_fdoa, z,C,x_init,alpha,beta,epsilon,max_num_iterations,force_full_calc,plot_progress,tdoa_ref_idx,fdoa_ref_idx)
...
if nargin < 12 || ~exist('plot_progress','var')
    plot_progress = false;
end

if nargin < 11 || ~exist('force_full_calc','var')
    force_full_calc = false;
end

if nargin < 10 || ~exist('max_num_iterations','var')
    max_num_iterations = [];
end

if nargin < 9 || ~exist('epsilon','var')
    epsilon = [];
end

if nargin < 8 || ~exist('beta','var')
    beta = [];
end

if nargin < 7 || ~exist('alpha','var')
    alpha = [];
end

% Initialize measurement error and Jacobian function handles
y = @(x) z - hybrid.measurement_TF(x_tdoa, x_fdoa, v_fdoa, x, tdoa_ref_idx, fdoa_ref_idx);
J = @(x) hybrid.jacobian_TF(x_tdoa, x_fdoa, v_fdoa, x, tdoa_ref_idx, fdoa_ref_idx);
...
C_tilde = blkdiag(utils.resampleCovMtx(C , test_idx_vec, ref_idx_vec));

[x,x_full] = utils.gdSoln_TF(y,J,C_tilde,x_init,alpha,beta,epsilon,max_num_iterations,force_full_calc,plot_progress);
