function [P,psi_vec] = music_modified(x,v,D,psi_LS,N_pts) % modidied for seraching around Coarse angle
%function [P,psi_vec] = music(x,v,D,max_psi,N_pts)  ----- Actual functuon
...
if nargin < 4 || isempty(psi_LS) %if nargin < 4 || isempty(max_psi) for grneral MUSIC
    psi_LS = pi/2;  % max_psi = pi/2;
end
...
psi_vec = linspace(psi_LS-1*pi/180,psi_LS+1*pi/180,N_pts);
