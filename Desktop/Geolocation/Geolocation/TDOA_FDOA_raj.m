...
v_sensor = [1, 1, 1.2; 1, 1, 0.7]*71;
x_sensor = [0, 40, 80; 0, 0, 0]*1e3;
%x_sensor2 = [60e3; 5e3];
%x_sensor3 = [150e3;10e3];
...
% Isochrones
dr1 = rngDiff(x_source,x_sensor(:,1),x_sensor(:,2));
%dr2 = rngDiff(x_source,x_sensor(:,2),x_sensor(:,3));
xiso1 = drawIsochrone(x_sensor(:,1),x_sensor(:,2),dr1,15000,700e3);
%xiso2 = drawIsochrone(x_sensor(:,2),x_sensor(:,3),dr2,15000,600e3);
plot(xiso1(1,:),xiso1(2,:),':','DisplayName','Line of Constant TDOA');
...
vdiff = dopDiff(x_source,[0 0]',x_sensor(:,1),v_sensor(:,1),x_sensor(:,2),v_sensor(:,2),3e8);
x_isodop = drawIsodop(x_sensor(:,1),v_sensor(:,1),x_sensor(:,2),v_sensor(:,2),vdiff,1500,700e3);
