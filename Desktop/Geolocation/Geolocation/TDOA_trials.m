...
L = chol(cov_r_out, 'upper'); %find L s.t cov_r_out = LL*, where L* is herimitian 
L2 = chol(cov_rr_out,'lower');
...
for i = 1:numSimulations
    ...
    noise2 = L2 * randn(size(L,2), 1);
    zeta_fdoa = z + noise2;
    % LS Solution
    x_ls_results(:,i) = tdoa.lsSoln(tdoa_sensors, zeta_noisy, cov_r, x_init);

    %x_ls_results_TDOA = tdoa.lsSoln(tdoa_sensors, zeta_noisy, cov_r, x_init);
    %x_ls_results(:, i) = fdoa.lsSoln(x_fdoa, v_fdoa, zeta_fdoa, cov_rr, x_ls_results_TDOA);
    %bootstrapping ftw
    %bootstrapping directly doesnt work.
