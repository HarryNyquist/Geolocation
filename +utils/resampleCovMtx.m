%Let there be N sensors. Now, just for simplicity, let's say that all the
%sensors are independent, and have a range rate error of sigma_1^2,
%sigma_2^2 ,...., up upto sigma_N^2. Now, the input, cov, is
%diag(sigma_1^2, sigma_2^2,...,sigma_N^2). Now, let's say that the first
%sensor is taken as reference. Then, the actual covariance matrix you would
%need (to set the error up, in the log likelihood function), is simply
%C_rho = (sigma_1^2)*(matrix with all entries = 1) + diag(sigma_2^2,
%sigma_3^2,..., sigma_N^2);

%This required C_rho is the output of the resampleCovMtx.
